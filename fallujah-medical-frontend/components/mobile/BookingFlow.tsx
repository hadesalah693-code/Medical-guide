...
import {
  ...
  consultationFee, experienceYears, generateTimeSlots, nextDays,
} from "@/lib/doctors";
import Button from "../ui/Button";
import RatingWidget from "../ui/RatingWidget";
import { IconX, IconPhone, IconWhatsApp, IconMapPin, IconClock, IconChevron } from "../ui/Icons";
...
export default function BookingFlow({ doctor, onClose }: BookingFlowProps) {
  ...
  if (step === "success") {
    return (
      <div className="fixed inset-0 z-[300] flex items-end lg:items-center justify-center lg:p-4">
        ...
        <div className="relative w-full modal-panel-lg bg-white rounded-t-3xl lg:rounded-2xl p-8 text-center animate-slide-up lg:animate-scale-in safe-bottom">
  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-end lg:items-center justify-center lg:p-4">
      ...
      <div className="modal-panel-lg relative animate-slide-up lg:animate-scale-in">
      {/* Header */}
      <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-100">
        <button type="button" onClick={step === "date" ? onClose : () => setStep(step === "confirm" ? "time" : "date")} className="w-9 h-9 rounded-full bg-gray-100 flex items-center justify-center">
          <IconChevron dir="left" size={20} />
        </button>
        <div className="flex-1">
          <h2 className="font-display font-bold text-[16px] text-gray-900">حجز موعد</h2>
          <p className="text-[12px] text-gray-500">{doctor.name}</p>
        </div>
      ...
      <div className="flex gap-1 px-4 py-3">
        {(["date", "time", "confirm"] as const).map((s, i) => (
          <div key={s} className={`h-1 flex-1 rounded-full transition-colors ${step === s || (step === "confirm" && i < 2) || (step === "time" && i === 0) ? "bg-primary-500" : "bg-gray-200"}`} />
        ))}
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-28">
        {step === "date" && (
          <div className="animate-fade-in-up">
              ...
              {days.map((d) => (
                <button
                  ...
                  className={[
                    ...
                    selectedDate === d.key ? "bg-primary-600 border-primary-600 text-white shadow-md" : "bg-white border-gray-200 text-gray-700",
                  ].join(" ")}
                >
            ...
            <Button full size="lg" className="mt-6" onClick={() => setStep("time")}>التالي</Button>
          </div>
        )}
        ...
        {step === "time" && (
          <div className="animate-fade-in-up">
              ...
              {slots.map((t) => (
                <button
                  ...
                  className={[
                    ...
                    selectedTime === t ? "bg-primary-600 border-primary-600 text-white" : "border-gray-200 text-gray-700 hover:border-primary-300",
                  ].join(" ")}
                >
                  {t}
                </button>
            ...
            <Button full size="lg" className="mt-6" disabled={!selectedTime} onClick={() => setStep("confirm")}>التالي</Button>
          </div>
        )}
        ...
        {step === "confirm" && (
          <div className="animate-fade-in-up">
            ...
            <Button full size="lg" className="mt-6" onClick={confirm}>تأكيد الحجز</Button>
          </div>
        )}
      </div>
      </div>
